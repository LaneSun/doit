...
pub async fn execute(_ctx: &RuntimeContext, args: &Args) -> Result<()> {
    ...
    let tmp_path = args.file.with_extension("tmp");
    if args.append && args.file.exists() {
        let existing = fs::read_to_string(&args.file).unwrap_or_default();
    ...
    fs::rename(&tmp_path, &args.file)
        .map_err(|e| crate::error::DoitError::io(e, format!("cannot rename to {}", args.file.display())))?;
